...
class MyTestCase(unittest.TestCase):
    ...
    def test_compare_two_approaches(self):
        ...
        # wind increment 5.0
        dt = 5.0
        x_arr = np.arange(10, 120, dt)
        y1 = single_exponential_given_V(beta_, alpha_, x_arr)
        y1_diff = np.diff(y1)
        y2 = vulnerability_weibull_pdf(x_arr[1:], alpha_, beta_) * dt

        # wind increment 1.0
        dt = 1.0
        x_arr_1 = np.arange(10, 120, dt)
        y1_1 = single_exponential_given_V(beta_, alpha_, x_arr_1)
        y1_diff_1 = np.diff(y1_1)
        y2_1 = vulnerability_weibull_pdf(x_arr_1[1:], alpha_, beta_) * dt

        # wind increment 0.5
        dt = 0.5
        x_arr_2 = np.arange(10, 120, dt)
        y1_2 = single_exponential_given_V(beta_, alpha_, x_arr_2)
        y1_diff_2 = np.diff(y1_2)
        y2_2 = vulnerability_weibull_pdf(x_arr_2[1:], alpha_, beta_) * dt
