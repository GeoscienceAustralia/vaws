...
def simulate_wind_damage_to_houses(cfg):
    ...
    # list_house_damage = ProcessingPool().map(create_house_damage,
    #                                          range(cfg.no_sims),
    #                                          [cfg] * cfg.no_sims)
    ...
    #         list_house_damage, [wind_speed]*cfg.no_sims)
        ...
        # compute average DI, and
        #print('{}'.format([x['di'] for x in list_]))
    ...
    logging.info('Elapsed time: {}'.format(time.time()-tic))
...
def save_results_to_files(cfg, list_results):
    ...
    # file_model (attribute: DataFrame(wind_speed_steps, no_sims)
