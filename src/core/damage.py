...
class WindDamageSimulator(object):
    ...
    def set_scenario(self, cfg):
        ...
        self.fragility_thresholds = self.set_fragility_thresholds(
    ...
    def simulator_mainloop(self, verbose=False):
        ...
        for id_sim in range(self.cfg.num_iters):
            self.id_sim = id_sim
            if not keep_looping:
                break
        ...
        if keep_looping:
            ...
            for id_speed, wind_speed in enumerate(self.speeds):
                ...
                perc = self.result_buckets['pressurized_count'].loc[id_speed]\
                       / float(self.cfg.num_iters) * 100.0
                ...
                for state, thr in self.cfg.fragility_thresholds.iteritems():
                    self.result_buckets['fragility'].loc[id_speed, state] = \
                        (self.result_buckets['dmg_index'].loc[id_speed] > thr
                         ).sum() / float(self.cfg.num_iters)
        ...
        for frag_ind, (state, value) in enumerate(
            ...
            try:
                ...
                self.file_frag.write('{:f},{:f}'.format(coeff_arr[0],
                                                        coeff_arr[1]))
                label = '{}({:.2f})'.format(state, value)
                self.fragility_thresholds.loc[state, 'object'] = \
                    CurvePlot(coeff_arr,
                              'lognormal',
                              self.cfg.wind_speed_min,
                              self.cfg.wind_speed_max,
                              label,
                              self.fragility_thresholds.loc[state, 'color'])
    ...
    def show_results(self, output_folder=None, vRed=40, vBlue=80):
        ...
        if self.cfg.flags['dmg_plot_vul']:
            ...
            output.plot_wind_event_show(self.cfg.num_iters,
                                        self.cfg.wind_speed_min,
                                        self.cfg.wind_speed_max,
                                        output_folder)
...
def simulate(cfg, options):
    ...
    if runTime:
        ...
        if options.plot_vuln:
            ...
            output.plot_wind_event_show(cfg.num_iters,
                                        cfg.wind_speed_min,
                                        cfg.wind_speed_max,
