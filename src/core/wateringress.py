'''
    Water Ingress Module - damage and costing aspects of water ingress based off Martin's work.
'''
import scipy.stats
import math
...
class WaterIngressCosting(database.Base):
    ...
    def __repr__(self):
        return 'WaterIngressCosting(%s; %.3f; $%.2f; %d; %.3f; %.3f; %.3f)' % (self.name,
                                                                               self.wi,
                                                                               self.base_cost,
                                                                               self.formula_type,
                                                                               self.coeff1,
...
def populate_water_costs(hid):
    ...
    damage_names = database.db.session.query(WaterIngressCosting.name).filter_by(house_id = hid).distinct().all()
...
def get_costing_for_envelope_damage_at_v(di, wind_speed, water_groups, out_file=None):
    ...
    watercosting = get_watercost_for_damage_at_wi(damage_name, water_ingress_perc)
    water_ingress_cost = 0.0
    if watercosting.formula_type == 1:
        water_ingress_cost = watercosting.base_cost * ((watercosting.coeff1 * math.pow(di, 2)) + (watercosting.coeff2 * di) + watercosting.coeff3)
    else:
        water_ingress_cost = watercosting.base_cost * watercosting.coeff1 * math.pow(di, watercosting.coeff2)

    if out_file:
        out_file.write('\n%f,%f,%f,%s,%f,%s' % (wind_speed, di, water_ratio, damage_name, water_ingress_cost, watercosting))
    ...
    class MyTestCase(unittest.TestCase):
        ...
        def test_water_costs(self):        
            ...
            for iter in xrange(n):
                i = 0
                for wi in wiarr:
                    watercost = get_watercost_for_damage_at_wi('Wall collapse', wi)
