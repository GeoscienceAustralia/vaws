...
class House(database.Base):
    ...
    conn_type_groups= relation(connection_type.ConnectionTypeGroup, order_by=connection_type.ConnectionTypeGroup.distribution_order)
    water_groups    = relation(connection_type.ConnectionTypeGroup, order_by=connection_type.ConnectionTypeGroup.water_ingress_order)
    ...
    def qryZoneByWallDirection(self, wall_dir):
        return database.db.session.query(zone.Zone).filter(zone.Zone.house_id == self.id).filter(zone.Zone.wall_dir == wall_dir).all()
        ...
        if lineCount != 0:
            ...
            for col, data in enumerate(fields):
                ...
                if col == 0:
                    ...
                    ins = db.structure_patch_table.insert().values(damaged_connection_id = damagedConn.id, 
...
# --------------------------------------------------------------
def queryHouses():
    return database.db.session.query(House.house_name, House.replace_cost, House.height).all()
...
def queryHouseWithName(hn):
    ...
    for ctg in house.conn_type_groups:
        # groups start out enabled unless the order is negative (this can be overridden later by scenario flags)
...
def loadConnectionTypesFromCSV(fileName, house):
    ...
    for row in x:
        tmp = connection_type.ConnectionType(row[0], row[6], row[1], row[2], row[3], row[4])
...
def loadDamageCostingsFromCSV(fileName, house):
    x = csvarray.readArrayFromCSV(fileName, "S50,f4,f4,i4,f4,f4,f4,f4,i4,f4,f4,f4")
    for row in x:
    ...
    for row in x:
        ...
        for ctg in house.conn_type_groups:
            ...
            if ctg.group_name == row[1]:
                factor_id = ctg.id                
        if parent_id == 0:
            raise LookupError('Invalid connection group name given: %s' % row[0])
        if factor_id == 0:
            raise LookupError('Invalid connection group name given: %s' % row[1])        
        tmp = damage_costing.DamageFactoring(parent_id=parent_id, factor_id=factor_id)
        ...
        if lineCount != 0:
            ...
            for col, data in enumerate(fields):
                ...
                if col == 0:
                    ...
                    inf.zone = childRight
                    #print 'adding influence from %s to %s of %f' % (parentLeft, childRight, float(data))
...
def loadCoverageTypesFromCSV(fileName, house):
    ...
    for row in x:
        tmp = CoverageType(name=row[0], failure_momentum_mean=row[1], failure_momentum_stddev=row[2])
...
def importDataFromPath(path):
    ...
    print house
    loadWallsFromCSV(path + '/walls.csv', house) ; print 'walls'
    loadCoverageTypesFromCSV(path + '/cov_types.csv', house) ; print 'coverage types'
    loadCoveragesFromCSV(path + '/coverages.csv', house) ; print 'coverages'
    loadDamageCostingsFromCSV(path + '/damage_costing_data.csv', house) ; print 'damage costings'
    loadWaterCostingsFromCSV(path + '/water_ingress_costing_data.csv', house) ; print 'water costings'
    loadConnectionTypeGroupsFromCSV(path + '/conn_group.csv', house) ; print 'connection type groups'
    loadConnectionTypesFromCSV(path + '/conn_type.csv', house) ; print 'connection types'
    loadDamageFactoringsFromCSV(path + '/damage_factorings.csv', house) ; print 'damage factorings'
    loadZoneFromCSV(path + '/zones.csv', house) ; print 'zones'
    loadConnectionsFromCSV(path + '/connections.csv', house) ; print 'connections' 
    loadConnectionInfluencesFromCSV(path + '/connectionzoneinfluences.csv', house) ; print 'connectionzoneinfluences'
    loadStructurePatchesFromCSV(path + '/influencefactorpatches.csv', house) ; print 'structureinfluencepatches'
