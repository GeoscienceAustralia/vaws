...
class ConnectionTypeGroup(database.Base):    
    ...
    costing                 = relation('DamageCosting', uselist=False, backref=backref('conn_types'))
...
class ConnectionType(database.Base): 
    ...
    house_id            = Column(Integer, ForeignKey('houses.id'))
    group               = relation(ConnectionTypeGroup, uselist=False, backref=backref('conn_types'))
    ...
    def __str__(self):
        return "(%s/%s)" % (self.group.group_name, self.connection_type)
    ...
    def sample_strength(self, mean_factor, cov_factor):
        real_mean = stats.lognormal_underlying_mean(self.strength_mean, self.strength_std_dev)
        real_stddev = stats.lognormal_underlying_stddev(self.strength_mean, self.strength_std_dev)
        ...
        return numpy.random.lognormal(mean, stddev)
...
#                        rv = numpy.random.lognormal(ct.strength_mean, ct.strength_std_dev)
#                        x.append(rv)
