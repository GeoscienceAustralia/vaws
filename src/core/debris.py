'''
    Debris Module - adapted from JDH Consulting and Martin's work
        - given  sim:wind_speed, sim:wind_dir, 
                ...
                scen:building_spacing, scen:debris_angle, scen:debris_radius, scen:flighttime_mean, scen:flighttime_stddev
...
import gendebrissrc, curve, engine, database
from rect import Rect, Point

# --- lookup table mapping (0-7) to wind direction desc    
dirs = ['S', 'SW', 'W', 'NW', 'N', 'NE', 'E', 'SE', 'Random']

# --- lookup table mapping wind direction (1-8) to list of front facing wall directions    
facing = { 1:[1], 2:[1,3], 3:[3], 4:[3,5], 5:[5], 6:[5,7], 7:[7], 8:[1,7]}
...
class DebrisType(object):
    def __init__(self, typeid, cdav, ratio, mass_mean, mass_stddev, fa_mean, fa_stddev, plot_shape):
...
class DebrisManager(object):
    def __init__(self, 
        ...
        if staggered_sources:
            posarr = gendebrissrc.genStaggered(debris_radius, debris_angle, building_spacing, False)
        else:
            posarr = gendebrissrc.genGrid(debris_radius, debris_angle, building_spacing, False)
        ...
        if region.name == 'Capital_city':
            self.dt_compact = DebrisType(0, 0.65, region.cr, region.cmm, region.cmc, region.cfm, region.cfc, 'o')
            self.dt_sheet = DebrisType(1, 0.9, region.pr, region.pmm, region.pmc, region.pfm, region.pfc, 's')
            self.dt_rod = DebrisType(2, 0.8, region.rr, region.rmm, region.rmc, region.rfm, region.rfc, 'd') 
        else:  
            self.dt_compact = DebrisType(0, 0.65, region.cr, region.cmm, region.cmc, region.cfm, region.cfc, 'o')
            self.dt_sheet =  DebrisType(1, 0.9, region.pr, region.pmm, region.pmc, region.pfm, region.pfc, 's')
            self.dt_rod = DebrisType(2, 0.8, region.rr, region.rmm, region.rmc, region.rfm, region.rfc, 'd')
    ...
    def set_wind_direction_index(self, wind_dir_index):
        ...
        if wind_dir_index in [0, 4]:
            rect = Rect(self.house.width, self.house.length, 0, self.debris_extension)
        elif wind_dir_index in [2, 6]:
            rect = Rect(self.house.length, self.house.width, 0, self.debris_extension)
        elif wind_dir_index in [1, 5]:
            rect = Rect(self.house.length, self.house.width, 45.0, self.debris_extension)
    ...
    def run(self, wind_speed, verbose=False):
        ...
        mean_prev = curve.single_exponential_given_V(A, wind_speed - self.wind_step)
        ...
        if item_mean > 0:
            ...
            num_itemsarr = numpy.random.poisson(item_mean, size=len(self.sources))
            
            # determine how many item buckets we need
            self.result_num_items = num_itemsarr.sum()
            self.result_scores = numpy.zeros(self.result_num_items)
    ...
    ## ------------------------------------------------------------------------
    def run_source(self, source, wind_speed, num_items, item_index, items, scores, verbose):
        ''' Returns number of impacts from this source. '''
        nv = 0
        for i in xrange(item_index, num_items+item_index):

            type = self.dt_sheet
            d100 = numpy.random.random_integers(1,100)
            if d100 <= self.region.cr:
                type = self.dt_compact
            elif d100 <= self.region.rr:
                type = self.dt_rod
            
            momentum, X, Y = engine.debris_generate_item(wind_speed, 
                                                         source.xord, source.yord,
                                                         self.flighttime_mean, 
                                                         self.flighttime_stddev,
                                                         type.typeid, 
                                                         type.cdav,
                                                         type.mass_mean, type.mass_stddev,
                                                         type.fa_mean, type.fa_stddev)
            
            # we only need to store items when we are running in verbose mode (for plotting)
            if verbose:
                item = DebrisItem(source.col, type.plot_shape) 
            ...
            for cov in wall.coverages:
                if not cov.result_intact:
                    if cov.description == 'window':
                        ...
                        thisarea = cov.result_num_impacts if cov.result_num_impacts <= cov.area else cov.area
                        area += thisarea
    ...
    def check_impacts(self):
        ...
        for wall in self.front_facing_walls:
            for cov in wall.coverages:
                q = cov.area
                Ed = engine.lognormal(cov.type.failure_momentum_mean, cov.type.failure_momentum_stddev)
                Cum_Ed = engine.percentileofscore(self.result_scores.tolist(), Ed)
                if cov.description == 'window':
                    Pd = 1.0 - numpy.exp( -Nv * (q/A) * (1.0 - Cum_Ed) )
    ...
    def render(self, v):
        ...
        title = 'Debris Sample Field for Wind Speed: %0.3f m/s in Region: %s' % (v, self.region.name)
